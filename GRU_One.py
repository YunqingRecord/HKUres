...
def Model():
    ...
    model = Sequential()
    model.add(GRU(30, batch_input_shape=(8, x_train.shape[1], x_train.shape[2]), stateful=True, return_sequences=True)) #, stateful=True))
    ...
    history = model.fit(x_train, y_train, epochs=3000, batch_size=8, shuffle=False)
    plt.figure(1)
